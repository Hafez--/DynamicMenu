...
@interface menuAnimationComponent()
...
@implementation menuAnimationComponent
...
- (void)setupMenuView {
    if (self.menuDirection == menuDirectionLeftToRight){
        ...
        NSLog(@"Left to Right");
        NSLog(@"InitialFrame: X=%f, Y= %f, Width=%f, Height=%f",self.menuInitialFrame.origin.x,self.menuInitialFrame.origin.y,self.menuInitialFrame.size.width, self.menuInitialFrame.size.height);

    }
    else if (self.menuDirection == menuDirectionRightToLeft){
    ...
    else if (self.menuDirection == menuDirectionDownToUp){
        self.menuInitialFrame = CGRectMake(self.menuFrame.origin.x,
...
- (void)setupSwipeGestureRecognizer {
    ...
    if (self.menuDirection == menuDirectionLeftToRight) {
        ...
        hideMenuGesture.direction = UISwipeGestureRecognizerDirectionDown;
    }
    else{
...
- (void)toggleMenu{
    ...
    if (!self.isMenuShown) {
        ...
        if (self.menuDirection == menuDirectionLeftToRight) {
            ...
            NSLog(@"Left to Right menuIsShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
            ...
            NSLog(@"Right To Left menuIsShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);

        }
        

        else if (self.menuDirection == menuDirectionUpToDown){
            // The value -1.0 means that gravity "pulls" the view towards the left side.
            ...
            // If the menu view is being shown from right to left, then the collision boundary
            // should be defined so as to be at the left of the initial menu view position.
            collisionPointFrom = CGPointMake(-self.menuFrame.origin.x, self.menuFrame.size.height);
            collisionPointTo = CGPointMake(self.menuFrame.size.width, self.menuFrame.size.height);

            NSLog(@"Up to Down menuIsShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
            ...
            collisionPointFrom = CGPointMake(self.menuFrame.origin.x,self.targetView.frame.size.height - self.menuFrame.size.height);
            collisionPointTo = CGPointMake(self.menuFrame.size.width,self.targetView.frame.size.height - self.menuFrame.size.height);
            
            NSLog(@"Down to Up menuIsShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
        ...
        if (self.menuDirection == menuDirectionLeftToRight) {
            ...
            NSLog(@"Left to Right menuIsNotShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
            ...
            collisionPointFrom = CGPointMake(self.targetView.frame.size.width + self.menuFrame.size.width, self.menuFrame.origin.y);
            collisionPointTo = CGPointMake(self.targetView.frame.size.width + self.menuFrame.size.width, self.menuFrame.size.height);
            NSLog(@"Right To Left menuIsNotShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
            ...
            gravityDirectionY = -1.0;
            gravityDirectionX = 0.0;
            collisionPointFrom = CGPointMake(-self.menuFrame.size.height, self.menuFrame.origin.x);
            collisionPointTo = CGPointMake(-self.menuFrame.size.height, self.menuFrame.size.width);
            
            NSLog(@"Up to Down menuIsNotShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
            ...
            collisionPointFrom = CGPointMake(self.menuFrame.origin.x, self.targetView.frame.size.height + self.menuFrame.size.height);
            collisionPointTo = CGPointMake(self.menuFrame.size.width, self.targetView.frame.size.height + self.menuFrame.size.height);
            NSLog(@"Down to Up menuIsNotShown");
            NSLog(@"collisionPoint:From X=%f, Y= %f, To X=%f, Y=%f",collisionPointFrom.x,collisionPointFrom.y,collisionPointTo.x, collisionPointTo.y);
    ...
    [gravityBehavior setGravityDirection:CGVectorMake(gravityDirectionX, gravityDirectionY)];
